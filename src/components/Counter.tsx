...
export type CounterProps = CounterInterface;

const Counter = (props: CounterInterface) => {

    const [one, setOne] = useState<number | undefined>(
        props?.min !== undefined && props.min > 9 ? Math.floor(props.min / 10) : undefined
    );
    const [two, setTwo] = useState<number>(
        props?.min !== undefined ? (props.min > 9 ? Math.floor(props.min % 10) : props.min) : 0
    );
    const [curr, setCurr] = useState<number>(props?.min ?? 0);
    ...
    return (
        ...
        >
            ...
            >
                <svg xmlns="http://www.w3.org/2000/svg" width={svg_size} height={svg_size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" className="icon icon-tabler icons-tabler-outline icon-tabler-minus"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M5 12l14 0" /></svg>
            </Button>
            <div
                style={{
...
export { Counter, Counter as AnimatedMotionCounter };
